...
class UserTestCases(APITestCase):
    ...
    def test_retrive_user_succes(self):
        ...
        self.client.login(username='testuser', email='testuser@gmail.com', password='testpassword')
        response = self.client.get(reverse('user_list', kwargs={'pk': self.user.id}), format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_retrive_user_fail(self):
        '''This function is used for retrive test case fail purpose'''
        self.client.login(username='testuser', password='testpassword')
        response = self.client.get(reverse('user_list', kwargs={'pk': 43}), format='json')
        self.assertEqual(response.status_code, 404)
    ...
    def test_update_user_fail(self):
        ...
        self.client.login(username='testuser', email='testuser@gmail.com', password='testpassword')
        response = self.client.patch(reverse('user_update', kwargs={'pk': 44}), self.data2 ,format ='json')
        self.assertEqual(response.status_code, 404)
    ...
    def test_update_user_pass(self):
        ...
        self.client.login(username='testuser',  password='testpassword')
        response = self.client.patch(reverse('user_update', kwargs={'pk': self.user.id}), self.data2 , format= 'json')
